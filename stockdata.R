...
usStocks <- filter(allStocks, Symbol %in%
stocks$symbol[stocks$country == "United States"])

write.csv(usStocks, "stocks_US.csv")

usStocksFiltered <- filter(allStocksFiltered,
Symbol %in% stocks$symbol[stocks$country == "United States"])
